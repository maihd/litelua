...
set LITELUA_LUAJIT_FOLDER=%LITELUA_DIR%\libs\luajit
set LITELUA_LUAJIT_INCLUDE=%LITELUA_LUAJIT_FOLDER%\include
set LITELUA_LUAJIT_LIBRARY=%LITELUA_LUAJIT_FOLDER%\prebuilt\win_x64\lua51_static.lib
...
if not exist "%LITELUA_LUAJIT_OUTPUT%\prebuilt\win_x64" (
    mkdir "%LITELUA_LUAJIT_OUTPUT%\prebuilt\win_x64"
)

xcopy "%LITELUA_DIR%\src\litelua.h" "%LITELUA_LUAJIT_OUTPUT%\include" /D /E /Y /Q
xcopy "%LITELUA_DIR%\src\litelua_config.h" "%LITELUA_LUAJIT_OUTPUT%\include" /D /E /Y /Q
